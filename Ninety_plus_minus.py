...
for i in np.arange(0,3):
    ...
    for m_ang in np.arange(1,90):

        for layers in np.arange(1,40):
            ...
            if tsai4>=1.5:
                total_thickness=2*(int(layers)*3)*th[i]
                # weight=.15*total_thickness*2*density[i]
                
                angles_dic[typ]=[m_ang,int(layers),tsai4,total_thickness,.15*total_thickness*density[i]]
